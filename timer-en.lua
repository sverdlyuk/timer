--[[
    Timer for Lilka
    LEFT/RIGHT - fied  |  UP/DOWN - value  |  A - Start/Pause  |  B - Exit
]]
...
function lilka.draw()
    ...
    -- Footer: B on the left, A with signature on the right
    local a_label
    if finished then
        a_label = "A: Reset"
    elseif running then
    ...
    display.set_cursor(25, H - 16)
    display.print("B: Exit")
    display.set_text_color(GREEN)
    display.set_cursor(W/2 + 20, H - 16)
